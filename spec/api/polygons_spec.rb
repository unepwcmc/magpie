...
describe "/polygons", type: :api do
  ...
  it "should create polygon" do
    ...
    post "/areas_of_interest/#{@area_of_interest.id}/polygons", "polygon[geometry][type]=#{geometry[:type]}&polygon[geometry][coordinates]=#{geometry[:coordinates]}"
  ...
  context "there is one polygon" do
    ...
    it "should update polygon" do
      # Work-around to parse correctly the geometry
      put "/polygons/#{@polygon.id}", "polygon[geometry][type]=#{geometry[:type]}&polygon[geometry][coordinates]=#{geometry[:coordinates]}"
